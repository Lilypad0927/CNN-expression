...
'''
this file analyzes results
'''

def analyze(predictions, reality):
	index_to_expression = ['Angry', 'Disgust', 'Fear', 'Happy', 'Sad', 'Surprise', 'Contempt']
	...
	for i in range(7):
		if len(guess_distribution_dict[i]) > 0:
			...
			print(float(guess_distribution_dict[i].count(i))) 
			print(len(guess_distribution_dict[i]))
